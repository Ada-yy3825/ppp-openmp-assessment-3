...
The scaling curve is non-linear across the thread ladder. In my measurements, the 16-thread run was slower than the serial baseline, which suggests that thread-management overhead, NUMA placement effects, and memory-system overhead dominate at low parallel occupancy. Performance improves significantly at 64 and 128 threads, where the stencil can better utilise the aggregate socket and full-node memory bandwidth. The strongest departure from ideal linear scaling occurs at 16 threads. Beyond that point, the kernel behaves more like a bandwidth-bound stencil, where memory-controller contention and NUMA effects dominate instead of pure compute throughput.
